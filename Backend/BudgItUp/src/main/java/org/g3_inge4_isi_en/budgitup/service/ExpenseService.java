    ...
    public class ExpenseService {
        ...
        public ExpenseDto updateExpense(Long userId, Long expenseId, ExpenseDto dto) {
            // 🔥 FIXED: Use findByIdAndUserId instead of findById
            Expense existing = expenseRepository.findByIdAndUserId(expenseId, userId)
                    .orElseThrow(() -> new EntityNotFoundException("Expense not found or does not belong to user"));
            ...
            if (dto.getCategoryId() != null && !existing.getCategory().getId().equals(dto.getCategoryId())) {
                Category newCat = categoryRepository.findByIdAndUserId(dto.getCategoryId(), userId)
        ...
        /**
         * Get total for a category (user-specific)
         * 🔥 CRITICAL FIX: Now requires userId to ensure user isolation
         */
        public double getCategoryTotal(Long userId, Long categoryId) {
            // Verify category belongs to user first
            Category category = categoryRepository.findByIdAndUserId(categoryId, userId)
                    .orElseThrow(() -> new EntityNotFoundException("Category not found or does not belong to user"));

            Double sum = expenseRepository.sumAmountByCategoryIdAndUserId(categoryId, userId);
